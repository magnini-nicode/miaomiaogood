...
public class EsSearchComponent {
    ...
    public void updateDocCount(String videoId,String fieldName,Integer count){
        try{
            ...
            Script script = new Script(ScriptType.INLINE,"painless","ctx_source"+fieldName+" += params.count", Collections.singletonMap("count",count));
            updateRequest.script(script);
            restHighLevelClient.update(updateRequest,RequestOptions.DEFAULT);
        }catch(Exception e){
            log.error("更新到es失败",e);
            throw new BusinessException("更新到es失败");
        }
    }
    ...
    public PaginationResultVO<VideoInfo> search(Boolean highlight,String keyword,Integer orderType,Integer pageNo,Integer pageSize){
        try {
            ...
            InfoQuery userInfoQuery = new InfoQuery();
            userInfoQuery.setUserIdList(userIdList);
            List<Info> userInfoList = infoMapper.selectList(userInfoQuery);
            Map<String,Info> userInfoMap =userInfoList.stream().collect(Collectors.toMap(item->item.getUserId(), Function.identity(),(data1,data2)->data2));

            videoInfoList.forEach(item->{
                Info userInfo = userInfoMap.get(item.getUserId());
            ...
            log.error("查询到es失败",e);
            throw new BusinessException("查询视频失败");
        }
